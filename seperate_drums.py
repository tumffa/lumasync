...
def separate_drums_with_larsnet(drum_audio_path, output_dir, wiener_filter=1.0, device="cpu"):
    ...
    larsnet_paths = [
        os.path.join(os.getcwd(), "larsnet"),
    ...
    try:
        ...
        # Initialize combined with kick and snare
        combined = kick_audio + snare_audio
        ...
        if 'toms' in output_paths:
            try:
                ...
                combined += toms_audio
                print(f"Added toms to combined audio")
        ...
        print(f"Combined shape: {combined.shape}, sr: {sr}")
        
        # Verify the data is valid
        if np.isnan(combined).any() or np.isinf(combined).any():
            print("Warning: Combined audio contains NaN or Inf values. Using original drums instead.")
            audio, sr = librosa.load(drum_audio_path, sr=None, mono=True)
            return audio, sr
            
        # Check if the result is too short
        if len(combined) < 1000:
            print("Warning: Combined audio is too short. Using original drums instead.")
            audio, sr = librosa.load(drum_audio_path, sr=None, mono=True)
            return audio, sr
        ...
        # Check if the combined data is too large and trim if needed
        max_safe_length = 5 * 60 * sr  # 5 minutes at the current sample rate
        if len(combined) > max_safe_length:
            print(f"Warning: Combined audio is very long ({len(combined)/sr:.1f} sec). Trimming to {max_safe_length/sr:.1f} sec.")
            combined = combined[:max_safe_length]

        # Save the combined audio to a file
        combined_output_path = os.path.join(song_output_dir, "combined.wav")
        wav.write(combined_output_path, sr, combined.astype(np.float32))
        print(f"Saved combined audio to {combined_output_path}")

        # Transform stereo to mono if needed
        if len(combined.shape) > 1 and combined.shape[1] == 2:
            print(f"Converting stereo audio to mono (shape was {combined.shape})")
            # Average the channels
            combined = np.mean(combined, axis=1)
            print(f"New shape: {combined.shape}")
        elif len(combined.shape) > 1:
            print(f"Warning: Audio has unusual shape: {combined.shape}, converting to mono")
            combined = np.mean(combined, axis=1)
            
        # Ensure it's a 1D array
        combined = combined.flatten()
        print(f"Final combined audio shape: {combined.shape}")
        return combined, sr
